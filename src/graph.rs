...
/*pub fn build_graph<B: Backend, O: Output<B>>(
    ...
    output.build(world, factory, &mut graph_builder, comp_subpass);
...
pub fn rendering_bundle<B: Backend, G: GraphCreator<B>>(
    builder: &mut Builder,
    world: &World,
    mut graph_creator: G,
) -> Result<(), Error> {
    ...
    let mut graph = graph_creator.build(world, &mut factory, &mut families)?;

    builder.add_thread_local_fn(move |world| {
        let mut factory = world
            .resources
            .get_mut::<Factory<B>>()
            .expect("factory was not inserted into world");

        if world.resources.get::<Exit>().is_none() {
            let mut families = world
                .resources
                .get_mut::<Families<B>>()
                .expect("families was not inserted into world");

            if graph_creator.rebuild(world) {
                graph.dispose(&mut factory, world);
                graph = graph_creator
                    .build(world, &mut factory, &mut families)
                    .expect("error creating graph");
            }
